...
// Hàm sắp xếp các nghiệm theo thứ tự giảm dần
...
void Eigendecomposite(double a[][MAX], int n) {
    ...
    cout << "\nEigenvalues (Ma trận A):\n";
    print(a, n); // Ma trận A

    double output[MAX];
    Solve(1, -a[0][0], -a[0][1], -a[0][2], output);
    sort(output, n); // Sắp xếp các giá trị riêng
    for (int i = 0; i < n; i++) {
        FormMatrix(output[i], C, P, n, i);
    }
    cout << "\nEigenvectors (Ma trận P):\n";
    print(P, n); // Ma trận P
}
