    ...
    void FormMatrix(double numb, double M[][MAX], double output[], int row) {
    ...
    void Eigendecomposite(double a[][MAX], int n) {
        double M[MAX][MAX], M1[MAX][MAX], temp[MAX][MAX], C[MAX][MAX];
        Identify(C, n);
        ...
        for (int i = 0; i < n; i++) {
            double vector[MAX];
            FormMatrix(output[i], C, vector, n);
            cout << "Vector " << i + 1 << ":\n";
            for (int j = 0; j < n; j++) {
                cout << fixed << setprecision(5) << vector[j] << endl;
            }
        }
    }
